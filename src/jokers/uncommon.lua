...
SMODS.Joker {
    ...
    loc_vars = function(self, info_queue, card)
        ...
        if info_queue then
            info_queue[#info_queue + 1] = { key = 'bull_market', set = 'Other' }
            info_queue[#info_queue + 1] = { key = 'bear_market', set = 'Other' }
        end
...
SMODS.Joker {
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local spin_str = (is_es and "Sin giros" or "None")
        if ex and ex.last_spin and #ex.last_spin == 3 then
            ...
            spin_str = (sym_map[ex.last_spin[1]] or ex.last_spin[1]) .. " | " ..
                       (sym_map[ex.last_spin[2]] or ex.last_spin[2]) .. " | " ..
                       (sym_map[ex.last_spin[3]] or ex.last_spin[3])
        end
    ...
    calculate = function(self, card, context)
        ...
        if context.after and not context.blueprint and card.ability.extra.bet_placed and not card.ability.extra.challenge_completed then
            ...
            if ch and ch.check(context) then
                ...
                ease_dollars(payout)
                local msg_txt = G.Witch_brew_SPANISH and ('¡Reto Cumplido! +$' .. payout) or ('Challenge Completed! +$' .. payout)
                ...
                if G.consumeables and #G.consumeables.cards < G.consumeables.config.card_limit then
                    G.E_MANAGER:add_event(Event({
                        ...
                        func = function()
                            play_sound('tarot1')
                            local job_keys = {
                                'c_Witch_brew_minero_job', 'c_Witch_brew_gardener_job', 'c_Witch_brew_banker_job',
    ...
    calculate = function(self, card, context)
        if context.end_of_round and not context.blueprint and not context.individual and not context.repetition then
            ...
            if pseudorandom('injured_joker') < (prob / odds) then
                local motorized_key = (G.P_CENTERS and G.P_CENTERS['j_Witch_brew_motorizado_joker']) and 'j_Witch_brew_motorizado_joker' or 'j_motorizado_joker'
                local msg_rock = G.Witch_brew_SPANISH and '¡A rockear!' or "Let's rock!"
                local msg_cursed = G.Witch_brew_SPANISH and '¡Maldito!' or 'Cursed!'
...
SMODS.Joker {
    ...
    config = { extra = { xmult_gain = 0.1, xmult = 1.0 } },
    rarity = 2,
    pos = { x = 5, y = 1 },
    cost = 6,
    blueprint_compat = true,
    loc_vars = function(self, info_queue, card)
        local ex = (card and card.ability and card.ability.extra) or self.config.extra
        return { vars = { ex.xmult_gain or 0.1, ex.xmult or 1.0 } }
    end,
    calculate = function(self, card, context)
        if context.before and not context.blueprint then
...
SMODS.Joker {
    ...
    config = { extra = { xmult_gain = 0.05, xmult = 1.0, odds = 6 } },
    rarity = 2,
    pos = { x = 6, y = 4 },
    cost = 6,
    blueprint_compat = true,
    loc_vars = function(self, info_queue, card)
        local ex = (card and card.ability and card.ability.extra) or self.config.extra
        local prob = (G.GAME and G.GAME.probabilities.normal) or 1
        return { vars = { ex.xmult_gain or 0.05, prob, ex.odds or 6, ex.xmult or 1.0 } }
    end,
    calculate = function(self, card, context)
        if context.discard and not context.blueprint then
            if context.other_card and context.other_card:is_face() then
                card.ability.extra.xmult = (card.ability.extra.xmult or 1.0) + (card.ability.extra.xmult_gain or 0.05)
                local prob = (G.GAME and G.GAME.probabilities.normal) or 1
                local will_destroy = pseudorandom('hoguera_destroy') < (prob / (card.ability.extra.odds or 6))
