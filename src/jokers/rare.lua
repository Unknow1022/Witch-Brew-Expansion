...
SMODS.Joker {
    ...
    loc_vars = function(self, info_queue, card)
        local ex = (card and card.ability and card.ability.extra) or self.config.extra
        local status = (ex and ex.guaranteed) and (G.Witch_brew_SPANISH and "¡Garantizado!" or "Guaranteed!") or (G.Witch_brew_SPANISH and "Pendiente" or "Pending")
        ...
        if context.end_of_round and not context.blueprint and not context.individual and not context.repetition then
            ...
            if G.jokers and G.jokers.cards and #G.jokers.cards > 0 then
                for i = 1, hands_played do
                    if pseudorandom('radiacion_debuff') < (prob / odds) then
                        local candidates = {}
                        for _, j in ipairs(G.jokers.cards) do
                        ...
                        local target = (#candidates > 0) and pseudorandom_element(candidates, pseudoseed('radiacion_target')) or pseudorandom_element(G.jokers.cards, pseudoseed('radiacion_target_all'))
                        if target then
