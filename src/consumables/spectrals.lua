...
SMODS.Consumable {
    ...
    loc_txt = {
        ...
        text = {
            ...
            "{C:inactive}(\"A MEMBER OF THE GANG HAS ARRIVED!!\"){}"
        }
    },
    in_pool = function(self, args)
    ...
    use = function(self, card, area, copier)
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                ...
                play_sound('foil1')
                local new_joker = create_card('Joker', G.jokers, nil, nil, nil, nil, chosen_key, 'the_gang')
                new_joker:add_to_deck()
                G.jokers:emplace(new_joker)
                new_joker:juice_up(0.8, 0.8)
                card_eval_status_text(card, 'extra', nil, nil, nil, { message = "A MEMBER OF THE GANG HAS ARRIVED!!", colour = G.C.DARK_EDITION })
                return true
            end
        }))
    end
...
SMODS.Consumable {
    ...
    use = function(self, card, area, copier)
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                play_sound('foil1')
                local new_joker = create_card('Joker', G.jokers, nil, nil, nil, nil, target_data.key, 'necromancy')
